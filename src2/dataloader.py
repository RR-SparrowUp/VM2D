#Dataloader for VMP2D
#Author: Reiyo 
#Organization: Sparrow Golf
#Date: 2025-03-21
#Description: This is a dataloader for VMP2D.
#It loads the data from the json file and returns a dataset.
#The dataset is a tuple of ground truth 2D keypoints , normalized view matrix, original view matrix, normalized 2D keypoints reshaped , normalized 3D keypoints.
...
class PoseDataLoader():
    ...
    def prepare_data(self, batch_size, is_training=True):
        ...
        output_types = (
            tf.float64,  # normalized_kpts
            tf.float64,  # normalized_view_matrix
            tf.float64,  # original_view_matrix
            tf.float64,  # keypoints_2d
            tf.float64   # keypoints_3d
        )
        
        output_shapes = (
            tf.TensorShape([input_dim]),  # normalized_kpts
            tf.TensorShape([4, 4]),  # normalized_view_matrix
            tf.TensorShape([4, 4]),  # original_view_matrix
            tf.TensorShape([num_joints, 2]),  # keypoints_2d
            tf.TensorShape([num_joints, 3])   # keypoints_3d
        )
